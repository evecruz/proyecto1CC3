...
int cantFeatures(int maximum_displacement){ //cantidad de features que podemos obtener (cuadro grande derecha)
      ...
      for(j = 0;j<image_height;j++){
        ...
        for(x=0;x<feature_width;x++){
	 	      for(y=0;y<feature_height;y++){
	 	   	   *cuadritoIzq=*(x*feature_width+y);//paso a vector el cuadrito
           		   *(cuadritoIzq++);
	 	   }
          	}
