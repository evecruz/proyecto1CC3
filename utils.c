...
/* 
 * Lee un little-endian int de un buffer (funciona tanto en  little como en big endian hosts)
 */
...
/* Carga un archivo bitmap file en FILENAME. */
Image load_image(char *filename) {
    ...
    if (// Se espera un 54-byte header
        readInt(info + 0x0e) != 40 ||
        // Se espera un bit-depth de 8 bits (grayscale)
        readInt(info + 0x1c) != 8
        ) {
    ...
    // Se asume 1 byte por pixel
    row_padding_per_row = (4 - ((width * 1) % 4)) % 4;

    // Cargando datos de abajo hacia arriba
...
/* Imprime el 2-D array de tamaño W * H almacenado en DATA. */
...
/* Código común para la escritura de archivos BMP. */
/* NOTA: Los pixeles deben ser {blue, green, red} en  ese orden, si la imagen es de 24 bits */
...
/* Guarda una imagen grayscale bitmap image en FILENAME */
